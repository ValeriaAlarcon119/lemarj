...
const TRENDS = (t: (key: string) => string) => [
  {
    title: t('newsBoost.trends.t0.title'),
    category: t('newsBoost.trends.t0.category'),
    growth: "+520%",
  ...
  {
    title: t('newsBoost.trends.t1.title'),
    category: t('newsBoost.trends.t1.category'),
    growth: "+310%",
  ...
  {
    title: t('newsBoost.trends.t2.title'),
    category: t('newsBoost.trends.t2.category'),
    growth: "+680%",
          ...
          <div className="max-w-2xl">
            ...
            <motion.h2 
              ...
              dangerouslySetInnerHTML={{
                __html: `<span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-purple-400 to-cyan-400">${t('newsBoost.title1')}</span><br />${t('newsBoost.title2')}`
              }}
            />
            <motion.p 
              initial={{ opacity: 0, y: 20 }}
              ...
              dangerouslySetInnerHTML={{
                __html: `${t('newsBoost.desc1')}<span class="text-foreground font-bold">${t('newsBoost.descBold')}</span>${t('newsBoost.desc2')}`
              }}
            />
          </div>
          <motion.div
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-white">
          {trendsData.map((item, index) => (
            ...
            >
              ...
              >
                <span>{t('newsBoost.readMore')}</span>
            ...
            >
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  <h3 className="text-3xl font-black text-white">{t('newsBoost.modalTitle')}</h3>
                  <p className="text-muted-foreground text-lg">
                    {t('newsBoost.modalDesc')}
                  </p>
                </div>

                <div className="p-8 rounded-3xl bg-indigo-500/5 border border-indigo-500/20 text-center space-y-4">
                  <p className="font-bold text-indigo-400 italic">{t('newsBoost.modalQuote')}</p>
                  <p className="text-sm text-muted-foreground">{t('newsBoost.modalFooter')}</p>
                </div>

                <div className="flex justify-center">
                  <Button size="lg" onClick={() => setIsModalOpen(false)}>
                    {t('newsBoost.modalBtn')}
                  </Button>
                </div>
              </div>
            </motion.div>
