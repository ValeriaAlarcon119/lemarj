        ...
        <div className="text-center max-w-4xl mx-auto mb-16">
          <motion.h2 
            ...
            dangerouslySetInnerHTML={{
              __html: `<span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-purple-400 to-cyan-400">${t('erpDashboard.title1')}</span> <br />${t('erpDashboard.title2')}`
            }}
          />
          <p 
            className="text-lg md:text-xl text-muted-foreground font-medium max-w-3xl mx-auto"
            dangerouslySetInnerHTML={{
              __html: `${t('erpDashboard.desc1')}<span class="text-foreground font-bold">${t('erpDashboard.descBold')}</span>${t('erpDashboard.desc2')}`
            }}
          />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white">{t('erpDashboard.advisor.title')}</h3>
                    <p className="text-sm text-gray-400">{t('erpDashboard.advisor.desc')}</p>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="p-4 rounded-2xl bg-gray-800/50 border border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-gray-400">{t('erpDashboard.advisor.incomeLabel')}</span>
                      <TrendingUp className="w-4 h-4 text-green-400" />
                    </div>
                    <div className="text-2xl font-bold text-white">{t('erpDashboard.advisor.incomeValue')}</div>
                    <div className="text-xs text-green-400 mt-1">{t('erpDashboard.advisor.incomeDiff')}</div>
                  </div>
                  
                  <div className="p-4 rounded-2xl bg-gray-800/50 border border-gray-700">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-gray-400">{t('erpDashboard.advisor.expenseLabel')}</span>
                      <TrendingDown className="w-4 h-4 text-red-400" />
                    </div>
                    <div className="text-2xl font-bold text-white">{t('erpDashboard.advisor.expenseValue')}</div>
                    <div className="text-xs text-red-400 mt-1">{t('erpDashboard.advisor.expenseDiff')}</div>
                  </div>
                </div>
          ...
          >
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl font-bold">{t('erpDashboard.recruitment.title')}</h3>
            </div>
            ...
            <div className="relative mb-6">
              ...
              <div className="w-full bg-background/50 border rounded-lg py-2 pl-10 pr-4 text-sm text-muted-foreground italic">
                {t('erpDashboard.recruitment.searchPlaceholder')}
              </div>
            </div>

            <div className="space-y-4 flex-1">
              {[
                { name: "Ana Maria G.", match: "98% Match", role: t('erpDashboard.recruitment.roles.r0') },
                { name: "Carlos Ruiz", match: "92% Match", role: t('erpDashboard.recruitment.roles.r1') },
                { name: "Diana Lopez", match: "89% Match", role: t('erpDashboard.recruitment.roles.r2') }
              ].map((candidate, i) => (
            ...
            <button className="w-full mt-6 py-3 bg-primary text-primary-foreground rounded-xl text-sm font-bold hover:opacity-90 transition-opacity">
              {t('erpDashboard.recruitment.publishBtn')}
            </button>
          </motion.div>
            ...
            <div className="relative h-full w-full bg-white/40 dark:bg-zinc-900/40 backdrop-blur-xl rounded-[calc(3rem-2px)] p-8 md:p-12 border border-border/50 flex flex-col md:flex-row items-center gap-12 overflow-hidden">
              <div className="flex-1 relative z-10">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <h3 className="text-3xl font-black text-foreground">{t('erpDashboard.inventory.title')}</h3>
                </div>
                <p className="text-muted-foreground text-lg leading-relaxed max-w-xl font-medium">
                  {t('erpDashboard.inventory.desc')}
                </p>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-6 relative z-10">
                <div className="bg-pink-500/5 dark:bg-pink-500/10 px-8 py-6 rounded-[2.5rem] border border-pink-500/20 shadow-sm text-center min-w-[160px] group/item hover:scale-105 transition-transform duration-500">
                  ...
                  <div className="text-[10px] text-pink-600 dark:text-pink-400 uppercase font-black tracking-[0.2em]">{t('erpDashboard.inventory.delayedStock')}</div>
                </div>
                <div className="bg-indigo-500/5 dark:bg-indigo-500/10 px-8 py-6 rounded-[2.5rem] border border-indigo-500/20 shadow-sm text-center min-w-[160px] group/item hover:scale-105 transition-transform duration-500">
                  ...
                  <div className="text-[10px] text-indigo-600 dark:text-indigo-400 uppercase font-black tracking-[0.2em]">{t('erpDashboard.inventory.averageRotation')}</div>
                </div>
                <div className="bg-zinc-950 dark:bg-white text-white dark:text-black px-10 py-6 rounded-[2.5rem] shadow-xl text-center min-w-[180px] flex flex-col justify-center group/item hover:scale-105 transition-transform duration-500 cursor-pointer border border-white/10 dark:border-black/10">
                  <div className="text-lg font-black leading-tight flex items-center justify-center gap-2">
                    {t('erpDashboard.inventory.freeCapital')} <ArrowUpRight className="w-4 h-4" />
                  </div>
                  <div className="text-xs opacity-70 mt-1 font-bold">{t('erpDashboard.inventory.suggestCombos')}</div>
                </div>
              </div>
